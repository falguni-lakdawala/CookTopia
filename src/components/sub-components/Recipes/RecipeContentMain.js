...
const RecipeContentMain = ({recipeData}) => {
    ...
    useEffect(() => {
        const target = document.querySelector(".recipe-fav-btn > svg");
        if (target && user) {
            const results = fetch(
                `http://44.238.74.165:3000/recipe/getfavoriterecipes/${user.uid}`).then(j => j.json()).then(d => {
                if (d.some(s => s.id === recipeData.id)) {
    ...
    const favRecipe = (e) => {
        ...
        const handlelike = () => {
            ...
            if (e.target.parentElement.classList.contains('active')) {
                ...
                }).then(j => j.json()).then(() => e.target.parentElement.classList.remove('active')).catch(e => console.log("Error updating like"))
            }

            // update like to the recipe
            if (user && !e.target.parentElement.classList.contains('active')) {
                fetch("http://44.238.74.165:3000/recipe/updaterecipelike", {
                    method: "PUT",
                    headers: {
